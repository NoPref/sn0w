...
int32_t ImGui_ImplAndroid_HandleTouchEvent(AInputEvent* event, ImVec2 screen_scale)
{
    ImGuiIO& io = ImGui::GetIO();
    ...
    case AMOTION_EVENT_ACTION_POINTER_DOWN:
        if (pointerId < IM_ARRAYSIZE(io.MouseDown))
        {
            io.MouseDown[pointerId] = true;
            ImVec2 pos(AMotionEvent_getX(event, pointerIndex), AMotionEvent_getY(event, pointerIndex));
    ...
    case AMOTION_EVENT_ACTION_MOVE:
        for (int i = 0; i < AMotionEvent_getPointerCount(event); i++)
        {
            ...
            if (pointerId < IM_ARRAYSIZE(io.MouseDown))
            {
                io.MousePos = ImVec2(screen_scale.x > 0 ? AMotionEvent_getX(event, pointerIndex) / screen_scale.x : AMotionEvent_getX(event, pointerIndex),
                                      screen_scale.y > 0 ? AMotionEvent_getY(event, pointerIndex) / screen_scale.y : AMotionEvent_getY(event, pointerIndex));
            }
        }
        return true;

    case AMOTION_EVENT_ACTION_SCROLL:
        io.MouseWheel = AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_VSCROLL, pointerIndex);
        io.MouseWheelH = AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_HSCROLL, pointerIndex);
        return true;
    }
    return false;
}
...
bool ImGui_ImplAndroid_Init(ANativeWindow* window)
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    io.BackendPlatformName = oxorany("imgui_impl_android");
